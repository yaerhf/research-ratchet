...
dispatches. Embedded verbatim in an agent definition, every `worker` dispatch
...
DST = ROOT / ".claude/agents/worker.md"

FRONT = """---
name: worker
description: >
  Fluent worker with FORMATION_CORE embedded as a byte-stable prefix
  ...
  by hand; regenerate with scripts/gen_worker_agent.py at every
  FORMATION_CORE version bump.
tools: *
model: inherit
---

You are a FLUENT WORKER on the programme. Your formation
...
def main() -> int:
    ...
    m = re.match(r"# FORMATION CORE — (v[\d.]+|TEMPLATE)", src)
    if not m:
