...
| 205 | **Query within your DIET BOUND.** Retrieval reaches the round directories under `knowledge/candidates/` — the derivations under review and every persisted verdict — so a starved role querying unbounded destroys its own measurement. Run the bound your brief's `[RETRIEVAL]` line names (`--diet starved` for the meta-observer, `--diet bare-statement` for the re-derivation agent; `open` only for saturated roles), and if you breached it, **say so in the verdict** rather than reporting a contaminated finding as clean. <sub>also in: WORKER, for any dispatch carrying a starvation</sub> | **ABSOLUTE** | Added 2026-08-27 with the retrieval layer: only `knowledge/audit/` is excluded from the index, so the starved roles' forbidden material is one unbounded query away — and the resulting verdict **looks identical**, which is the signature of the failure class this apparatus most cares about (*a separation asserted and never verified is a convention, not a control*) |
